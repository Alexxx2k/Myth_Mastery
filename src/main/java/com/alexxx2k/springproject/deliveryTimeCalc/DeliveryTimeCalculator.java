...
public class DeliveryTimeCalculator {
    ...
    private static final PropertiesLoader properties = new PropertiesLoader();

    public static Long getMinutes(String cityName) {

        try {
            String lat = Double.toString(DGISGeocoder.getCoordinates(cityName, properties.get2GisApiKey())[0]);
            String lon = Double.toString(DGISGeocoder.getCoordinates(cityName, properties.get2GisApiKey())[1]);
